 ...
 * @author Lab-CB08
 */
@WebServlet(name = "FitrarPorNombre", urlPatterns = {"/FitrarPorNombre"})
public class FitrarPorNombre extends HttpServlet {
ResultSet cdr = null;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FitrarPorNombre</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FitrarPorNombre at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     processRequest(request, response);
        ...
        String nombre = request.getParameter("nombre");
            String comando = "select folio,nombre_completo,asunto,fecha,estado from agenda where nombre_completo like '%"+nombre+"%' and estado='pendiente';";
            try {
                cdr = sentenciaSQL.executeQuery(comando);
                out.println("<table width=100% border=1 >");
                out.println("<tr>");
                out.println("<th>Folio</th><th>Nombre</th><th>Asunto</th><th>Fecha</th><th>Estado</th><th></th>");
                out.println("</tr>");
                while (cdr.next()) {
                    
                    out.println("<tr>");
                    out.println("<td width=40%><input type=\"hidden\" id=\"folio\" name=\"folio\" value="+cdr.getString(1)+">" + cdr.getString(1) + "</td>");
                    out.println("<td width=40%><input type=\"hidden\" id=\"nombre\" name=\"nombre\" value="+nombre+">" + cdr.getString(2) + "</td>");
